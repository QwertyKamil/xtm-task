...
const App = () => {
    ...
    useEffect(() => {
        if (searchPhrase) {
            setIsSearching(true);
            axios.get(`https://en.wikipedia.org/w/api.php?action=query&list=search&format=json&srsearch=%22${searchPhrase}%22&srlimit=10`)
                .then(({data: {query: {search}}}) => {
    ...
    return (
        <>
            <div>
                <input ref={inputEl} type="text" onChange={debounce(function (e) {
                    setSearchPhrase(inputEl.current.value);
                }, 1000)}/>
                <input type="text" onChange={({target: {value}}) => {
                    setReplacePhrase(value)
                }}/>
                <button disabled={isSearching} onClick={() => setSearch(state => !state)}>Search</button>
                <button disabled={isSearching} onClick={() => {
